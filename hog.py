...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def announcement(score_0, score_1):
        if who == 0:
            whos = score_0
        else:
            whos = score_1
        difference = whos - previous_score
        if difference>previous_high and difference == 1:
            print(str(difference) + ' point!','That\'s the biggest gain yet for Player ' + str(who))
        elif difference>previous_high and difference > 1:
            print(str(difference) + ' points!','That\'s the biggest gain yet for Player ' + str(who))
        if difference>previous_high:
            maximum = difference
        else:
            maximum = previous_high
        return announce_highest(who, maximum, whos)
    return announcement
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    high = 0
    while trial <= 10:
        total = make_averaged(roll_dice, num_samples)
        var = total(trial, dice)
        if var > high:
            high = var
            count = trial
        trial += 1
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    zero = abs(opponent_score%10 - opponent_score//10) + 2
    if zero >= margin:
        return 0
    return num_rolls
    # END PROBLEM 10
